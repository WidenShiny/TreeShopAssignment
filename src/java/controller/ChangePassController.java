...
public class ChangePassController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String renew_pass = request.getParameter("renewpass");
        AccountDAO accountDAO = new AccountDAO();

        if (old_pass.equals(account.getPassword())) {
            if (new_pass.equals(renew_pass)) {
                accountDAO.changePass(new_pass, account.getUsername());
                request.setAttribute("changesuccess", "Change password successfully");
            } else {
                request.setAttribute("checknewpass", "Please enter new password match with re-new password!!!");
            }
        } else {
            request.setAttribute("checkoldpass", "Please enter correct old password!!!");
        }
