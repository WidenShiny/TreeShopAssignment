    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        for (int i = 0; i < listProducts.size(); i++) {
            if (i >= ((page - 1) * 6) && i < (page * 6)) {
                listProducts1.add(listProducts.get(i));
            }
        }

        int totalPage = listProducts.size() / PAGE_SIZE;
        if (listProducts.size() % PAGE_SIZE != 0) {
            totalPage += 1;
        }

        request.setAttribute("listCategories", listCategories);
        request.setAttribute("category", "filtercategory");
        request.setAttribute("page", page);
        request.setAttribute("totalPage", totalPage);
        request.setAttribute("listProducts", listProducts1);
